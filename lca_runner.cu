...
const string help_msg = "Command line arguments\n \
  ...
  -a [cuda-inlabel, cuda-naive, cpu-inlabel, cpu-rmq, cpu-simple, multicore-cpu-inlabel] -- chooses algorithm to run\n";

typedef void (*CpuLcaFunction)(int N, const int *parents, int Q, const int *queries, int *answers);
typedef void (*MCpuLcaFunction)(int N, const int *parents, int Q, const int *queries, int *answers, int batchSize);
...
unordered_map<string, MCpuLcaFunction> m_cpu_algorithms = {{"multicore-cpu-inlabel", &multicore_cpu_lca_inlabel}};
...
int main(int argc, char *argv[]) {
  ...
  MCpuLcaFunction m_cpu_algorithm_to_use = NULL;
  ...
  while ((opt = getopt(argc, argv, "hb:i:o:a:")) != EOF) {
    ...
    case 'a': {
      ...
      else if (m_cpu_algorithms.count(alg) > 0)
        m_cpu_algorithm_to_use = m_cpu_algorithms[alg];
  ...
  } else if (m_cpu_algorithm_to_use != NULL) {
    m_cpu_algorithm_to_use(tc.tree.V, tc.tree.father.data(), tc.q.N, tc.q.tab.data(), answers.data(), batchSize);
